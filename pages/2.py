...
df='tips-displot.png'
plt.savefig(df)
with open(df, 'rb') as img:
    st.sidebar.download_button(
            label='Скачать график Распеределения', data=img, file_name=df, mime='image/png')
...
with open(df, 'rb') as img:
    st.sidebar.download_button(
            label='Скачать график Зависимостей', data=img, file_name=df, mime='image/png')
